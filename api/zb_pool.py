...
@api_bp.route('/zb-pool', methods=['GET'])
def get_zb_pool():
    """
    获取炸板股票池
    ...
    try:
        ...
        stocks = ZbgcService.get_zbgc_pool(date=api_date)
        ...
        if save_to_db:
            try:
                db = next(get_db())
                saved_count = ZbgcPoolHistoryService.save_today_zbgc_pool(db)
                return jsonify({
                    'success': True,
                    'data': stocks,
                    'count': len(stocks),
...
@api_bp.route('/zb-pool', methods=['POST'])
def save_zb_pool():
    """
    保存当前炸板股票池到数据库
    """
...
@api_bp.route('/zb-pool/history', methods=['GET'])
def get_zb_pool_history():
    """
    获取炸板股票池历史数据
...
@api_bp.route('/zb-pool/export', methods=['POST'])
def export_zb_pool():
    """导出炸板股票池到Excel"""
    ...
    try:
        excel_file = export_zbgc_to_excel(date=date)
        return jsonify({
            'success': True,
            'message': '导出成功',
            'file': excel_file
