...
from utils.dtgc_excel_export import export_dtgc_to_excel
from database.db import get_db
from datetime import datetime, date

@api_bp.route('/dt-pool', methods=['GET'])
def get_dt_pool():
    """
    获取跌停股票池
    ...
    try:
        ...
        stocks = DtgcService.get_dtgc_pool(date=api_date)
        ...
        if save_to_db:
            try:
                db = next(get_db())
                saved_count = DtgcPoolHistoryService.save_today_dtgc_pool(db)
                return jsonify({
                    'success': True,
                    'data': stocks,
                    'count': len(stocks),
...
@api_bp.route('/dt-pool', methods=['POST'])
def save_dt_pool():
    """
    保存当前跌停股票池到数据库
    """
...
@api_bp.route('/dt-pool/history', methods=['GET'])
def get_dt_pool_history():
    """
    获取跌停股票池历史数据
...
@api_bp.route('/dt-pool/export', methods=['POST'])
def export_dt_pool():
    """导出跌停股票池到Excel"""
    ...
    try:
        excel_file = export_dtgc_to_excel(date=date)
        return jsonify({
            'success': True,
            'message': '导出成功',
            'file': excel_file
